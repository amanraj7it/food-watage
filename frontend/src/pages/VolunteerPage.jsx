...
export default function VolunteerPage({ user, donations = [], onSaveDonations, isEmbedded = false }) {
    const defaultVolunteerUser = initialUsers.find(u => u.role === 'volunteer') || {
        id: 'msvph9xe',
        name: 'Bhakti Tyagi',
    ...
    const [currentUser, setCurrentUser] = useState(user || defaultVolunteerUser);
        ...
        if (!user) {
            ...
            if (s) {
                try {
                    ...
                    if (parsed.role && parsed.role === 'volunteer') {
                    ...
                    fetch('/api/users')
                        ...
                        .then(d => {
                            ...
                            if (found && (found.role === 'volunteer' || found.role === 'admin')) {
                ...
                setCurrentUser(defaultVolunteerUser);
            }
        } else {
            setCurrentUser(user);
        }
